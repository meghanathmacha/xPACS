...
perKcomputation <- function(k,iter,packs,Econstant,f,data) 
{
  ...
  simcpacks = lapply(1:iter,function(x){return(Reduction(packids = simfpacks[[x]],
                                                         packs,data,Econstant =Econstant,f = f) - logstar(k))})
...
computePart3Parallel <- function(name, precision = 3, dom = TRUE, iter = 10, trainid)
{
  ...
  baseline = list()
  nanoms = nrow(data[data$label == 0,])
  nanoms = 7 # Hard coding the number of packs.
  packedanoms = 0
...
precision = 3
f = 10^precision
setwd("/nfshome/SHARED/BreastCancerData/BaseLines/Qual/Part2")
cv = 1
for(i in 1:cv)
{
  result = computePart3Parallel("breast",precision = 3,dom = TRUE,iter = 25,trainid = i)
  saveRDS(result,"resultbreast.RDS")
